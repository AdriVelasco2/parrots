...
const LoginForm = () => {
  ...
  const displayMsg = () => {
    toast.error(
      "Este correo no tiene ninguna reserva registrada, si consideras que es un error ponte en contacto con nosotros.",
      {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      }).base("appyreXbsqMvaSsZe"); // Reemplaza con tu API Key y Base ID
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        ...
        <main class=" p-10 mx-auto flex min-h-screen w-full items-center justify-center  text-[#e3d779]">
          <section class="flex w-[30rem] flex-col space-y-10">
            ...
            <button
              class="transform rounded-md bg-[#f0dc46] text-black py-2 font-bold duration-300 hover:bg-lime-300 hover:text-black"
              type="submit"
            >
              QUIERO FIESTA
            </button>

            <a class="transform text-center font-semibold text-[#e3d779] p-4 rounded-md border-2 border-[#e3d779] bg-black bg-opacity-70 duration-300 hover:text-gray-300">
              Introduce el correo electrónico con el que realizaste tu reserva
            </a>
          </section>
        </main>
      </form>
      <a
        href="/"
        className="fixed top-6 left-6 z-50 bg-[#e3d779] text-black px-4 py-2 rounded-full shadow-lg font-bold hover:bg-lime-400 transition-colors duration-200"
        title="Volver atrás"
      >
        ← Volver
      </a>
